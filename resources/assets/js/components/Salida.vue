

<template>
        <main class="main">
            <!-- Breadcrumb -->
            <audio id="xyz" preload="auto"  src="audio/error.mp3" type="audio/mpeg" > </audio>
            <ol class="breadcrumb">
                
              <br>
            </ol>
            <div class="container-fluid">
                <!-- Ejemplo de tabla Listado -->
                <div class="">
                    <div class="card-header hedCa white-text">
                        <div class="row">
                            <div class="col">
                                <span class="div" v-if="listado==1 || listado==0">
                                    <i class="fa fa-align-justify"></i> Salida
                                </span>
                                <span class="div" v-if="listado==2">
                                    <i class="fas fa-people-carry"></i> Devolucion
                                </span>
                                <span class="div" v-if="listado==3">
                                    <i class="fas fa-truck-moving"></i> Liquidacion
                                </span>
                            </div>
                            <div class="col sm-0 sp-0">
                                <button 
                                    v-show="listado==0 || listado==2 || listado==3" @click="ocultarDetalle()"
                                    type="button" class="redondear2 btn-transparent whit-de float-right" >
                                    <i class="fas fa-external-link-alt"></i> Atras
                                </button>
                                <div class="visible-wE meGr sm-0 float-right">
                                    
                                    <button 
                                        v-show="listado==1" @click.prevent="listado=2"
                                        type="button" class="redondear2 btn-transparent whit-de " >
                                        <i class="fas fa-people-carry"></i> Devolucion
                                    </button>
                                    <button 
                                        v-show="listado==1" @click.prevent="listado=3"
                                        type="button" class="redondear2 btn-transparent whit-de " >
                                        <i class="fas fa-truck-moving"></i> Liquidacion
                                    </button>
                                    <button 
                                        v-show="listado==1" @click="mostrarDetalle()"
                                        type="button" class="redondear2 btn-transparent whit-de " >
                                        <i class="fas fa-external-link-alt"></i> Nuevo
                                    </button>
                                </div>
                                <div class="visible-rE float-right">
                         
                                    <div v-if="listado==1" class="d-flex flex-column text-md-center"   >

                                            <button v-if="vme==0" @click="vme=1" title="Opcion y edicion"  
                                                    type="button" class="redondear2 btn-transparent whit-de" >
                                                <i class="fas fa-cog"></i>
                                            </button>
                                            <button v-if="vme==1" @click="vme=0" title="Opcion y edicion"  
                                                    type="button" class="redondear2 btn-transparent whit-de" >
                                                <i class="fas fa-cog"></i>
                                            </button>
                                    </div>

                                </div>
                                
                            </div>
                        </div>

                    </div>
                        <div v-if="vme==1" class="visible-rE  triangulo_sup">

                        </div>
                        <div v-if="vme==1"  class="bg-mee redondear2 visible-rE mt-2 mb-2 p-2">
                            <div class="mb-2 text-center">
                                <button 
                                    v-show="listado==1" @click.prevent="listado=2,vme=0"
                                    type="button" class="redondear2 btn-transparent whit-de " >
                                    <i class="fas fa-people-carry"></i> Devolucion
                                </button>
                                <button 
                                    v-show="listado==1" @click.prevent="listado=3,vme=0"
                                    type="button" class="redondear2 btn-transparent whit-de " >
                                    <i class="fas fa-truck-moving"></i> Liquidar
                                </button>
                                <button 
                                    v-show="listado==1" @click="mostrarDetalle(),vme=0"
                                    type="button" class="redondear2 btn-transparent whit-de " >
                                    <i class="fas fa-external-link-alt"></i> Nuevo
                                </button>
                            </div>
                        </div>
 
                    <!-- Lista -->
                     <template v-if="listado==1">
                        <div class="card-body">
                            <br>
                            <b-tabs pills card>
                                <b-tab title="Equipos" active>

                                    <div class="">
                                        <a >
                                            <div class="div p-2 white b-01 bg-dev black-text ml-2">
                                            <i class="fas fa-box"></i> Total Registro : {{contaTotalPr}}
                                            </div>
                                        </a>
                                    </div>
                                    
                                    <div class="bg-grasy p-2 mt-4 card">
                                        <div class="row">
                                            <div class="col-md-7 pt-2">
                                                <div class="row">
                                                    <div class="col-md-4">
                                                        <select class="form-control" v-model="criterio">
                                                            <option value="ctdetsa_serie">Serie Producto</option>
                                                            <option value="ctdetsa_sap">Sap</option>
                                                            <option value="ctdetsa_fecha_act">Fecha</option>
                                                        </select>
                                                    </div>
                                                <div class="my-4 visible-rE"></div>
                                                <div class="col-md-7" >
                                                    <input type="text" v-model="buscar" @keyup="listarSalida(buscar,criterio)" class="form-control" placeholder="Texto a buscar">
                                                </div>
                                                <div class="col-1 visible-wE">
                                                    <button :disabled="buscar == ''" @click="listarSalida(buscar,criterio)" type="button" class="psoFSal btn green btn-sm ">
                                                        <i  class="fa fa-search"></i>
                                                    </button>
                                                </div>
                                                </div>
                                            
                                            </div>
                                            <div class="col">
                                                <div class="p-2 d-flex flex-row-reverse">
                                                    <a @click="pdfExportEquipo()">
                                                        <div class=" p-2 white b-01  ml-2">
                                                        <i class="fas fa-file-pdf red-text"></i> Imprimir Pdf
                                                        </div>
                                                    </a>
                                                    <a  @click="excelExportEquipo()" >
                                                        <div class=" p-2 b-01 white">
                                                            <i class="fas fa-file-excel green-text"></i> Imprimir Excel
                                                        </div>
                                                    </a>
                                                </div>
                                            </div>
                                        </div>
                                    </div>

                                    <div class="table-responsive">
                                        <table class="table table-bordered table-striped table-sm">
                                            <thead class="hedCa white-text text-center">
                                                <tr>
                                                    
                                                    <th>N°</th>
                                                    <th>Producto</th>
                                                    <th>SAP</th>
                                                    <th>Serie Producto</th>
                                                    <th>Cantidad</th>
                                                    <th>Unidad de Medida</th>
                                                    <th>Fecha Registro</th>
                                                    <th>Tipo de Producto</th>
                                                    <th>Usuario</th>
                                                    
                                                
                                                
                                                </tr>
                                            </thead>
                                            <tbody>
                                                <tr v-for="(salida,indice) in (arraySalida)" :key="salida.id" >
                                                    
                                                    <!-- <td v-text="salida.ctdetsa_salid_nro"></td> -->
                                                    <td>{{indice+1}}</td>
                                                    <td v-text="salida.ctdetsa_produDescri"></td>    
                                                    <td v-text="salida.ctdetsa_sap"></td>
                                                    <td v-text="salida.ctdetsa_serie"></td>
                                                    <td v-text="salida.ctdetsa_cantidad"></td>
                                                    <td v-text="salida.ctdetsa_undmdDescri"></td>
                                                    <td v-text="salida.ctdetsa_fecha_reg"></td>
                                                    <td v-text="salida.ctdetsa_descriTipo"></td>
                                                    <td v-text="salida.ctdetsa_usuario"></td>
                                                    
                                                </tr>
                                            </tbody>
                                        </table>
                                    </div> 
                                    
                                    <nav aria-label="pagination example">
                                        <ul class="pagination pg-blue">
                                            <li class="page-item" v-if="pagination.current_page > 1">
                                                <a class="page-link" href="#" @click.prevent ="cambiarPagina(pagination.current_page-1,buscar,criterio)">Ant</a>
                                            </li>
                                            <li class="page-item" v-for="page in pagesNumber" :key="page" :class="[page==isActived ? 'active' : '']">
                                                <a class="page-link" href="#" @click.prevent="cambiarPagina(page,buscar,criterio)" v-text="page" ></a>
                                            </li>
                                        
                                            <li class="page-item" v-if="pagination.current_page<pagination.last_page">
                                                <a class="page-link" href="#" @click.prevent="cambiarPagina(pagination.current_page+1,buscar,criterio)">Sig</a>
                                            </li>
                                        </ul>
                                    </nav>
                                </b-tab>
                                <b-tab title="Materiales">
                                    
                                    <div class="">
                                    <a >
                                        <div class="div p-2 white b-01 bg-dev black-text ml-2">
                                        <i class="fas fa-box"></i> Total Registro : {{contaTotalPr}}
                                        </div>
                                    </a>
                                </div>
                                    <div class="bg-grasy p-2 mt-4 card">
                                        <div class="row">
                                            <div class="col-md-7 pt-2">
                                                <div class="row">
                                                    <div class="col-md-4">
                                                        <select class="form-control" v-model="criterioSW">
                                                            <option value="ctdetsa_sap">Sap</option>
                                                            <option value="ctdetsa_fecha_act">Fecha</option>
                                                        </select>
                                                    </div>
                                                <div class="my-4 visible-rE"></div>
                                                <div class="col-md-7" >
                                                    <input type="text" v-model="buscarSW" @keyup="listarSalidaMaterial(buscarSW,criterioSW)"  class="form-control" placeholder="Texto a buscar">
                                                </div>
                                                <div class="col-1 visible-wE">
                                                    <button :disabled="buscarSW == ''" @keyup="listarSalidaMaterial(buscarSW,criterioSW)"  type="button" class="psoFSal btn green btn-sm ">
                                                        <i  class="fa fa-search"></i>
                                                    </button>
                                                </div>
                                                </div>
                                            
                                            </div>
                                            <div class="col">
                                                <div class="p-2 d-flex flex-row-reverse">
                                                    <a @click="pdfExportMaterial()">
                                                        <div class=" p-2 white b-01  ml-2">
                                                        <i class="fas fa-file-pdf red-text"></i> Imprimir Pdf
                                                        </div>
                                                    </a>
                                                    <a  @click="excelExportMaterial()" >
                                                        <div class=" p-2 b-01 white">
                                                            <i class="fas fa-file-excel green-text"></i> Imprimir Excel
                                                        </div>
                                                    </a>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    

                                    <div class="table-responsive">
                                        <table class="table table-bordered table-striped table-sm">
                                            <thead class="hedCa white-text text-center">
                                                <tr>
                                                    
                                                    <th>N°</th>
                                                    <th>Producto</th>
                                                    <th>SAP</th>
                                                    <th>Cantidad</th>
                                                    <th>Unidad de Medida</th>
                                                    <th>Fecha Registro</th>
                                                    <th>Tipo de Producto</th>
                                                    <th>Usuarios</th>
                                                    
                                                
                                                </tr>
                                            </thead>
                                            <tbody>
                                                <tr v-for="(salida,indice) in (arraySalidaMaterial)" :key="salida.idProducto"  >
                                                
                                                    <td >{{indice+1}}</td>
                                                    <td v-text="salida.ctdetsa_produDescri"></td>    
                                                    <td v-text="salida.ctdetsa_sap"></td>
                                                    <td v-text="salida.ctdetsa_cantidad"></td>
                                                    <td v-text="salida.ctdetsa_undmdDescri"></td>
                                                    <td v-text="salida.ctdetsa_fecha_reg"></td>
                                                    <td v-text="salida.ctdetsa_descriTipo"></td>
                                                    <td v-text="salida.ctdetsa_usuario"></td>
                                                    <!-- <td >
                                                        <b-btn type="button" class="btn btn-danger btn-sm" @click="desactivarEquipo(salida.ctdetsa_indice)">
                                                            Liquidar
                                                        </b-btn>
                                                    </td> -->
                                                </tr>
                                            </tbody>
                                        </table>
                                    </div>
                                    
                                    <nav aria-label="pagination example">
                                        <ul class="pagination pg-blue">
                                            <li class="page-item" v-if="pagination.current_page > 1">
                                                <a class="page-link" href="#" @click.prevent ="cambiarPaginaMaterial(pagination.current_page-1,buscarSW,criterioSW)">Ant</a>
                                            </li>
                                            <li class="page-item" v-for="page in pagesNumber" :key="page" :class="[page==isActived ? 'active' : '']">
                                                <a class="page-link" href="#" @click.prevent="cambiarPaginaMaterial(page,buscarSW,criterioSW)" v-text="page" ></a>
                                            </li>
                                        
                                            <li class="page-item" v-if="pagination.current_page<pagination.last_page">
                                                <a class="page-link" href="#" @click.prevent="cambiarPaginaMaterial(pagination.current_page+1,buscarSW,criterioSW)">Sig</a>
                                            </li>
                                        </ul>
                                    </nav>
                                </b-tab>
                                
                            </b-tabs>
                            
                        </div>
                    </template>
                     <!-- Salida -->
                    <template v-if="listado==0">
                        <div class="card-body">
                            <div class="row m-2">
                                
                                <div class="col-md-4">
                                    <label for="">Usuario
                                        
                                    </label>
                                        <v-select
                                        :on-search="selectUsuario"
                                        :class="{
                                            stylosError:this.usuario_id=='',
                                            stylosCorrec:this.usuario_id!=''
                                            }"
                                        label="ctusuar_usuario" 
                                        :options="arrayUsuario"
                                        placeholder="Buscar usuarios..."
                                        
                                        :onChange="getDatosProducto"                                        
                                    >
                                    
                                    </v-select>

                                    
                                </div>

                                <div class="col-md-4">
                                    <label for="">Fecha de Llegada</label>
                                    <datepicker v-model="fechaLlegada" format="dd/MM/yyyy" input-class="form-control">
                                    </datepicker>

                                </div>
                                <div class="col-md-4">
                                    <div class="p-4 d-flex flex-row-reverse">
                                        <button data-toggle="modal" 
                                                data-target="#basicExampleModal"
                                                :disabled="arrayUsuario.length < 1" 
                                                class="btn btn-sm orange text-center">
                                        
                                            <div class="div p-2 b-01 ">
                                            <i class="icon-plus"></i> Ingresar 
                                            </div>
                                       
                                        </button>
                                        
                                    </div>
                                </div>
                            </div>     
                            <!-- clase 4 si sirve -->

                                <!-- Modal -->
                                <div data-backdrop="static" data-keyboard="false" class="modal fade" id="basicExampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                    <div class="modal-dialog" role="document">
                                        <div class="modal-content">
                                        <div class="modal-header hedCa white-text">
                                            <h5 class="modal-title" id="exampleModalLabel">Salida de Productos</h5>
                                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                            <span aria-hidden="true" class="white-text">&times;</span>
                                            </button>
                                        </div>
                                
                                    <div hidden="hidden" id="tabla" class="card m-2" ref="myModalRef" >
                                        <input type="text" placeholder="Texto a buscar">
                                    <div class="table-responsive">
                                        <table   class="table table-bordered table-striped table-sm">
                                            <thead class="blue white-text text-center">
                                                <tr>
                                                    <th>SAP</th>
                                                    <th>SERIE</th>
                                                    <th>Acción</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                <tr v-for="salida in arraySalida" :key="salida.idProducto"  >
                                                    <td v-text="salida.ctdetsa_sot" ></td>
                                                    <td v-text="salida.ctdetsa_salid_nro"></td>
                                                    <td>
                                                        <button :disabled="codigo_sot == ''" type="button" class="btn btn-primary m-2 float-right btn-sm" data-toggle="modal" data-target="#basicExampleModal1">Agregar</button>
                                                    </td>
                                                </tr>
                                            </tbody>
                                        </table>
                                    </div>
                                    </div> 
                                    
                                        <div class="modal-body">
                                            <b-tabs pills card >
                                                <b-tab title="Equipo" active>
                                                    <div class="bg-grasy">
                                                        <div class="header-tableR">
                                                            <div class="triangulo_bottom_Activo">
                                                                <div class="text-center b-01 white-text pt-2">
                                                                    <i class="fas fa-ellipsis-v fl-rt"></i> Serie
                                                                </div>

                                                            </div>
                                                        
                                                        </div>
                                                        <div class="body-tableR">
                                                            <input type="text" v-model="buscar05"  @keyup="buscarInfo()" class="form-control " >
                                                        </div>
                                                    </div>

                                                    

                                            
                                                    
                                                
                                                    
                                                </b-tab>
                                                <b-tab title="Materiales">
                                                    <div class="bg-grasy">
                                                        <div class="header-tableR">
                                                            <div class="row">
                                                                <div class="col">
                                                                    <div class="triangulo_bottom_Activo">
                                                                        <div class="text-center b-01 white-text pt-2">
                                                                            <i class="fas fa-ellipsis-v fl-rt"></i> SAP
                                                                        </div>

                                                                    </div>
                                                                </div>
                                                                <div class="col">
                                                                    <div class="float-right">
                                                                        <div class="lineaH"></div>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        </div>
                                                        <div class="body-tableR">
                                                            <div class="row">
                                                                <div class="col-md-4">
                                                                    <div class="flex-center p-2">
                                                                        <span class=" b-01 text-center">{{materialName}}</span>
                                                                    </div>
                                                                    
                                                                </div>
                                                                <div class="col-md-8">
                                                                    <div class="row">
                                                                        <div class="col-10 ">
                                                                            <input v-b-tooltip.hover 
                                                                                    title="Presione ''ENTER''' para la búsqueda " 
                                                                                    type="text" v-model="buscar1" 
                                                                                    v-on:keyup.enter="listarMaterialB(buscar1,criterio2)" 
                                                                                    @keyup.enter="eqLisCanti=1"  class="form-control" 
                                                                                    placeholder="Texto a buscar">

                                                                        </div>
                                                                        <div class="col">
                                                                            <button  @click="listarMaterialB(buscar1,criterio2)"
                                                                                    :disabled="buscar1 == ''" 
                                                                                    type="button" 
                                                                                    class="psoFSal btn green btn-sm">

                                                                                <i  class="fas fa-search"></i>
                                                                            </button>
                                                                        
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        </div>
                                                        
                                                    </div>
                                                    <div class="body-tableR mt-2">
                                                        <div class="row">
                                                            <div class="col-md-6">
                                                                <div class="div" v-for="ingreso in arraySAPMate" :key="ingreso.pro">

                                                                    <label for="">Can.:</label>
                                                                    <span class="b-01">{{stockMaterial}}</span>
                                                                    <span class="green-text">{{ingreso.ctundmd_desc}}</span>
                                                                    <span class="float-right blue-text"> {{ingreso.stockMaximo}}</span>
                                                                    <b-progress :value="controlT(stockMaterial,ingreso.stockMaximo)" class="mb-3"></b-progress>
                                                                    

                                                                </div>

                                                                
                                                            </div>
                                                            <div class="col-4">
                                                            
                                                                <b-form-input   
                                                                            v-model="cantidadProductos"
                                                                            type="number"
                                                                            :disabled="buscar1 == ''||this.arraySAPMate ==''"
                                                                            :state="cantiState"
                                                                            
                                                                            >
                                                                </b-form-input>

                                                            </div>
                                                            <div class="col-2">
                                                                <button @click="agregra()" 
                                                                        :disabled="buscar1 == ''||this.arraySAPMate ==''||ConCanti" v-on:click="eqLisCanti=2" 
                                                                        type="button"
                                                                        class="psoFSal btn blue btn-sm">
                                                                    <i  class="fas fa-dolly"></i>
                                                                </button>
                                                            </div>
                                                        </div>
                                                    </div>
                                                                

                                                    <div class="row">    

                                                        <div class="col-md-12">
                                                            <div >
                                                                <br>
                                                            <hr class="my-1">
                                                            
                                                                <!-- User Interface controls -->
                                                                <b-row>
                                                                <b-col md="6" class="my-1">
                                                                    
                                                                    <label for="">Búsqueda</label>
                                                                    <b-input-group>
                                                                        <b-form-input v-model="filter" placeholder="buscar..." />
                                                                        <b-input-group-append>
                                                                        
                                                                        </b-input-group-append>
                                                                    </b-input-group>
                                                                
                                                                </b-col>
                                                                
                                                                <b-col md="6" class="my-1">
                                                                        <label for="">Paginación</label>
                                                                    <b-form-select :options="pageOptions2" v-model="perPage2" />
                                                    
                                                                </b-col>
                                                                </b-row>
                                                                <div class="table-responsive">
                                                                        <br>
                                                                        
                                                                        <!-- Main table element -->
                                                                        <b-table show-empty
                                                                                stacked="md"
                                                                                :items="arrayMaterialSap"
                                                                                :fields="fields01"
                                                                                :current-page="currentPage2"
                                                                                :per-page="perPage2"
                                                                                :filter="filter"
                                                                                :sort-by.sync="sortBy"
                                                                                :sort-desc.sync="sortDesc"
                                                                                :sort-direction="sortDirection"
                                                                                @filtered="onFiltered"
                                                                        >
                                                                        
                                                                        <template slot="OPCIONES" slot-scope="row">
                                                                            <!-- We use @click.stop here to prevent a 'row-clicked' event from also happening -->
                                                                            <b-button size="sm" @click.stop="info01(row.item, row.index, $event.target)"  class="mr-1">
                                                                            Agregar
                                                                            </b-button>
                                                                        </template>
                                                                        
                                                                        </b-table>
                                                                        
                                                                        <b-row>
                                                                        <b-col md="6" class="my-1">
                                                                            <b-pagination :total-rows="this.arrayMaterialSap.length" :per-page="perPage2" v-model="currentPage2" class="my-0" />
                                                                        </b-col>
                                                                        </b-row>

                                                                        <!-- Info modal -->
                                                                </div>

                                                            </div>
                                                            <div v-if="eqLis01 === 2" >
                                                                <br>
                                                                <b-container fluid>
                                                                    <div class="col-md-12"> 
                                                                    </div>  
                                                                </b-container>
                                                            </div>
                                                        </div>

                                                    </div>
                                                </b-tab>
                                            </b-tabs>
                                            
                                        </div>
                                    
                                        </div>
                                    </div>
                                    </div>

                                <div class="bg-grasy p-2  card">
                                    <div class="row">
                                        <div class="col">
                                            <div class="p-2">
                                                <a >
                                                    <div class="div p-2 white b-01 bg-dev black-text ml-2">
                                                    <i class="fas fa-box"></i> Total Equipos : {{contaTotal}}
                                                    </div>
                                                </a>
                                            </div>
                                        </div>
                                        <div class="col">
                                            <div class="p-2 d-flex flex-row-reverse">
                                                <a  :class="{disNo:tasks.length == 0 ,disBlo:tasks.length > 0 }" @click.prevent="registrarSalida()">
                                                    <div class="div p-2 blue b-01 bg-dev white-text ml-2">
                                                    <i class="fas fa-undo-alt"></i> Registrar entrada
                                                    </div>
                                                </a>
                                                <a  :class="{disNo:tasks.length == 0 ,disBlo:tasks.length > 0 }" @click.prevent="exportPdf" >
                                                    <div class="div p-2 red b-01 white">
                                                        <i class="fas fa-download"></i> Imprimir
                                                    </div>
                                                </a>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                
                                    <div class="table-responsive">
                                    <table class="table table-striped table-bordered " >
                                        <thead class="hedCa white-text">
                                            <tr>
                                                <th scope="col">#</th>
                                                <th scope="col">tipo</th>
                                                <th scope="col">Categoria</th>
                                                <th scope="col">Producto</th>
                                                <th scope="col">Serie del Producto</th>
                                                
                                                <th  scope="col">Cantidad</th>
                                                <th>Unidad Medida</th>
                                                <th scope="col">Opciones</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                                <tr v-for=" (task,indice) in (tasks) " :key="task.descripcion" >
                                                    <td>{{indice+1}}</td>
                                                    <td>

                                                        {{tipoDe(task.detaTi)}}
                                                        
                                                    </td>
                                                    <td>{{task.cate01}}</td>
                                                    <td>{{task.prodescri}}</td>
                                                    <td>{{task.prod01}}</td>
                                                    
                                                    <td class="retroCanti text-center">{{task.cantidadProducto}}</td>
                                                    <td>{{task.unidadDescripcion}}</td>
                                                    <td class="text-center ">
                                                        <div class="btn-group " >
                                                            <button type="button"  class=" btn-transparent red-text" @click="deleteTask(indice)">
                                                            <i class="fas fa-backspace"></i>
                                                            </button>
                                                        </div>
                                                    </td>
                                                

                                            </tr>
                                        </tbody>
                                    </table> 
                                </div>
                            
                        
                            <br>
                            <hr>
                            <br>
                        
                        </div>
                    </template>

                    <!-- devolucion -->
                    <template  v-if="listado==2">
                        <div class="card-body" >
                            <div class="row " v-if="devLis==0">
                                <div class="col-md-6">
                                    <div class="bg-grasy p-2">
                                        <div class="row">
                                                
                                                <div class="col-md-5" >
                                                    <input type="text" v-model="campoInputdevolucion" @keyup="listarSerieEquipo(campoInputdevolucion)" class="form-control" placeholder="Texto a buscar">
                                                </div>
                                                <div class="col-md-1">
                                                    <button type="button" class="visible-wE psoFSal btn green btn-sm ">
                                                        <i  class="fa fa-search"></i>
                                                    </button>
                                                </div>
                                                
                                                    <button type="button" @click.prevent="devLis=1" class=" psoFDev btn orange btn-sm ">
                                                    <i class="far fa-eye-slash"></i> 
                                                    </button>
                                                
                                        </div>   
                                        
                                    </div>
                                </div>
                            </div>
                            <div class="row">
                                
                                <div class="col-md transA" v-if="devLis==0" >
                                    <div class="card p-2">
                                        <div  class="table-responsive">
                                            <table @click="mostrarLista()" class="table table-bordered table-striped table-sm">
                                                <thead class="hedCa white-text text-center">
                                                    <tr class="text-center">
                                                        <th>#</th>
                                                        <th>Código. SAP</th>
                                                        <th>Código del producto</th>
                                                        <th>Fecha de Registro</th>
                                                        <th>Usuario</th>
                                                    
                                                    </tr>
                                                </thead>
                                                <tbody>
                                                    
                                                    <tr v-for="(salida,indice) in arrayListarSerie" :key="salida.idseriesalida"  >
                                                        <td>{{indice+1}}</td>
                                                        <td v-text="salida.ctdetsa_sap" ></td>
                                                        <td v-text="salida.ctdetsa_serie"></td>
                                                        <td >{{salida.ctdetsa_fecha_reg}}</td> 
                                                        <td >{{salida.ctdetsa_traspaso}}</td>
                                                        
                                                        
                                                        
                                                    </tr>
                                                    
                                                </tbody>
                                            </table>
                                            <b-btn v-show="arrayEntrada.length!==0" v-if="LiquiChecked.length !==0"  @click="liquidarProductos(LiquiChecked)"  variant="warning btn-sm" >
                                                    <i class="fa fa-trash black"></i>    
                                            </b-btn> 
                                        </div>
                                    </div>
                                </div>
                                
                                <div class="col-md-2 " v-if="devLis==1">
                                    <div class="bg-grasy">
                                        <div class="p-2 d-flex flex-row-reverse">

                                            <a @click.prevent="devLis=0">
                                                <div class="div p-2 red b-01 white">
                                                    <i class="far fa-eye"></i> Visible
                                                </div>
                                            </a>
                                        </div>
                                    </div>
                                   
                                </div>
                                
                                <div class="col-md transA">
                                    <div class="bg-grasy card">
                                        <div class="header-tableR">
                                            <div class="triangulo_bottom_Activo">
                                                <label class="text-center b-01 white-text pt-2 pl-4" for="">SERIE</label>
                                            </div>
                                        </div>
                                        <div class="body-tableR">
                                            <input class="form-control" 
                                                    v-model="serieInput" 
                                                    @keyup="agregarSerie(serieInput)" type="text">
                                        </div>
                                    </div>

                                    <div  :class="{disNo:this.probarinsertDevolucion=='' ,disBlo:this.probarinsertDevolucion!=''}">
                                        <div class="bg-grasy card">
                                            <div class="p-2 d-flex flex-row-reverse">
                                                <a  @click="devolucionProductos(probarinsertDevolucion)">
                                                    <div class="div p-2 blue b-01 bg-dev white-text ml-2">
                                                    <i class="fas fa-undo-alt"></i> Devolver
                                                    </div>
                                                </a>
                                                <a  @click="probarinsertDevolucion=[]" >
                                                    <div class="div p-2 red b-01 white ml-2">
                                                        <i class="fas fa-eraser"></i> Limpiar
                                                    </div>
                                                </a>
                                                <a @click.prevent="exportPdfDevolucion()" >
                                                    <div class="div p-2 red b-01 white">
                                                        <i class="fas fa-download"></i> Imprimir
                                                    </div>
                                                </a>
                                            </div>
                                        </div>
                                    </div>
                                    <hr class="mb-2">
                                    <div class="bg-grasy my-2 card" v-for="(series, index) in probarinsertDevolucion" :key="index">
                                        <div class="cerraDev" @click="eliminarIndexDevolucion(index)">
                                            <i class="fas fa-times white-text"></i>
                                        </div>
                                        <div class="header-tableR">
                                            <div class="tri_liqi">
                                                <label class="text-center b-01 white-text pt-2 pl-4" for="">{{index+1}}</label>
                                            </div>
                                        </div>
                                        <div class="body-tableR">
                                            <div class="d-flex flex-row">
                                                <div class="div">
                                                    <i class="far fa-circle green-text"></i>
                                                    <span class="b-01 ml-2 fontS-18">
                                                        {{series.Numeroserie}}
                                                    </span>
                                                </div>
                                                <div class="fot-right ">
                                                    <i class="far fa-user-circle orange-text"></i>
                                                    <span class="b-01 ml-2 fontS-18">
                                                        {{series.UserD}}
                                                    </span>
                                                </div>
                                            </div>
                                            <div class="ml-4 ">
                                                <i class="icoC-text">Codigo sap:</i>
                                                <span class="b-01 brown-text">
                                                    {{series.sap}}
                                                </span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>

  
                        </div>
                    </template>
                    <template  v-if="listado==3">
                        <div class="card-body" >
                            <div class="row " v-if="devLis==0">
                                <div class="col-md-6">
                                    <div class="bg-grasy p-2">
                                        <div class="row">

                                                <div class="col-md-5" >
                                                    <input type="text" v-model="inputbuscarliqui" @keyup="getDiasdatos(inputbuscarliqui)" class="form-control" placeholder="Texto a buscar">
                                                </div>
                                                <div class="col-md-1">
                                                    <button type="button" class="visible-wE psoFSal btn green btn-sm ">
                                                        <i  class="fa fa-search"></i>
                                                    </button>
                                                </div>
                                                
                                                    <button type="button" @click.prevent="devLis=1" class="psoFDev btn orange btn-sm ">
                                                    <i class="far fa-eye-slash"></i> 
                                                    </button>
                                                
                                        </div>   
                                        
                                    </div>
                                </div>
                            </div>
                            <div class="row">
                                
                                <div class="col-md transA" v-if="devLis==0" >
                                    <div class="card p-2">
                                         <span class="b-01 black-text m-2"><i  class="orange-text fas fa-exclamation-triangle mr-2"></i> Los productos se mostraran pasados los 13 dias y que no esten despachados</span>
                                            
                                        <div  class="table-responsive">
                                            <table @click="mostrarLista()" class="table table-bordered table-striped table-sm">
                                                <thead class="hedCa white-text text-center">
                                                    <tr class="text-center">
                                                        <th>#</th>
                                                        <th>Código. SAP</th>
                                                        <th>Código del producto</th>
                                                        <th>Fecha de Registro</th>
                                                    
                                                    </tr>
                                                </thead>
                                                <tbody>
                                                    
                                                    <tr v-for="(detpr,indice) in arrayListarSerieLimite" :key="detpr.idseriesalida"  >
                                                        <td>{{indice+1}}</td>
                                                        <td v-text="detpr.sap" ></td>
                                                        <td v-text="detpr.series"></td>
                                                        <td >{{detpr.fecha}}</td>
                                                        
                                                        
                                                        
                                                    </tr>
                                                    
                                                </tbody>
                                            </table>
                                            <b-btn v-show="arrayEntrada.length!==0" v-if="LiquiChecked.length !==0"  @click="liquidarProductos(LiquiChecked)"  variant="warning btn-sm" >
                                                    <i class="fa fa-trash black"></i>    
                                            </b-btn> 
                                        </div>
                                    </div>
                                </div>
                                
                                <div class="col-md-2 " v-if="devLis==1">
                                    <div class="bg-grasy">
                                        <div class="p-2 d-flex flex-row-reverse">

                                            <a @click.prevent="devLis=0">
                                                <div class="div p-2 red b-01 white">
                                                    <i class="far fa-eye"></i> Visible
                                                </div>
                                            </a>
                                        </div>
                                    </div>
                                   
                                </div>
                                
                                <div class="col-md transA">
                                    <div class="bg-grasy card">
                                        <div class="header-tableR">
                                            <div class="triangulo_bottom_Activo">
                                                <label class="text-center b-01 white-text pt-2 pl-4" for="">SERIE</label>
                                            </div>
                                        </div>
                                        <div class="body-tableR">
                                            <input class="form-control" 
                                                    v-model="serieInputLiqui" 
                                                    @keyup="liquidacionmetodo(serieInputLiqui)" type="text">
                                        </div>
                                    </div>

                                    <div  :class="{disNo:this.arrayliquida=='' ,disBlo:this.arrayliquida!=''}">
                                        <div class="bg-grasy card">
                                            <div class="p-2 d-flex flex-row-reverse">
                                                <a  @click="liquidacionProductos(arrayliquida)">
                                                    <div class="div p-2 blue b-01 bg-dev white-text ml-2">
                                                    <i class="fas fa-undo-alt"></i> Liquidar
                                                    </div>
                                                </a>
                                                <a  @click="arrayliquida=[]" >
                                                    <div class="div p-2 red b-01 white ml-2">
                                                        <i class="fas fa-eraser"></i> Limpiar
                                                    </div>
                                                </a>
                                                <a @click.prevent="exportPdfLiquidacion()" >
                                                    <div class="div p-2 red b-01 white">
                                                        <i class="fas fa-download"></i> Imprimir
                                                    </div>
                                                </a>
                                            </div>
                                        </div>
                                    </div>
                                    <hr class="mb-2">
                                    <div class="bg-grasy my-2 card" v-for="(liquida, index) in arrayliquida" :key="index">
                                        <div class="cerraDev" @click="eliminarIndexliquida(index)">
                                            <i class="fas fa-times white-text"></i>
                                        </div>
                                        <div class="header-tableR">
                                            <div class="tri_liqi">
                                                <label class="text-center b-01 white-text pt-2 pl-4" for="">{{index+1}}</label>
                                            </div>
                                        </div>
                                        <div class="body-tableR">
                                            <div class="d-flex flex-row">
                                                <div class="div">
                                                    <i class="far fa-circle green-text"></i>
                                                    <span class="b-01 ml-2 fontS-18">
                                                        {{liquida.serie}}
                                                    </span>
                                                    
                                                </div>
                                            </div>
                                            <div class="ml-4 ">
                                                <i class="icoC-text">Codigo sap:</i>
                                                <span class="b-01 brown-text">
                                                     {{liquida.sap}}
                                                </span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>

  
                        </div>
                    </template>
                </div>
                <!-- Fin ejemplo de tabla Listado -->
            </div>
     

            
           
        </main>
</template>

<script>
 
 const $ = require('jquery')
 window.$ = $
 //

//AUTOSELECIONAR UN IMPUT//
function pasacampo(key_event)
{
  var k;
  if (document.all)  k = event.keyCode;
  else k = key_event.which;
 if (k == 13) document.getElementById('campo2').focus();
}
    import jsPDF from 'jspdf';
    import autoTable from 'jspdf-autotable';

    import Datepicker from 'vuejs-datepicker';
    import moment from "moment";
    import vSelect from 'vue-select';

    const items = [
    
    ]
    
    export default {
        data(){
            return{
                // finds: [],
                items: items, 
                fields: ['sap','Serie','opciones'],
                fields01: ['SAP','DESCRIPCION','STOCK','OPCIONES'],
                fields02 :['ctdetsa_sot',
                            'ctdetsa_salid_nro',
                            'ctdetsa_produDescri',
                            'ctdetsa_sap',
                            'ctdetsa_serie',
                            'ctdetsa_cantidad',
                            'ctdetsa_undmdDescri',
                            'ctdetsa_fecha_reg',
                            'ctdetsa_descriTipo',
                            'ctdetsa_usuario',
                            ],
                currentPage: 1,
                currentPage1: 1,
                currentPage2: 1,
                perPage: 10,
                perPage1:5,
                perPage2:5,
                totalRows: items.length,
                totalRows1: items.length,
                pageOptions1: [ 5, 10 ],
                pageOptions2: [ 5, 10 ],
                pageOptions: [ 5, 10, 15 ],
                sortBy: null,
                sortDesc: false,
                sortDirection: 'asc',
                filter: null,
                modalInfo: { title: '', content: '' },
                devLis:0,

                eqLisCanti:2,
                eqLis:2, 
                usuario_id:0,
                codigo_sot:'',
                dateToFilter:moment(),
                fecha1:'',
                listado:1,
                descripcion:'',
                idcategoria:0,
                cantidad:0,
                cantidadMaterial:0, 
                unidadMedida:{},
                fechaRegistro:new Date(),
                fechaLlegada:new Date(),
                codigo:0,
                nombre_categoria:0,
                descripcion:'',
                caracteristicas:'',
                stock:0,
                modal:0,
                tituloModal:'',
                tipoAccion:0,
                errorEntrada:0,
                errorMostrarMsjEntrada:[],
                pagination:{
                    'total':0,
                    'current_page':0,
                    'per_page':0,
                    'last_page':0,
                    'from':0,
                    'to':0,
                },
                userSelet:[],
                newCantidad:[]=1,
                offset:3,
                criterio:'ctdetsa_serie',
                criterio1:'ctdetpr_serie',
                criterio2:'sap',
                criterioSW:'ctdetsa_sap',
                buscar:'',
                buscar1:'',
                buscarSW:'',
                buscarSerie:'',
                arrayCategoria:[],
                arraySalida:[],
                arraySalidaMaterial:[],
                newTasks:[],
                tasks:[],
                tasks1:[],
                arrayProducto:[],
                arraySAP:[],
                arrayProducto1:[],
                arrayMedida:[],
                arraySerie:[],
                arrayUsuario:[],
                arraySAPMate:[],
                arrayEquiEn:[],
                arrayListarSerie:[],
               arrayMaterialSap:[],
                agUni:'',
                errorIngreso:0,
                col:0,
                pro:1,
                CatePro01:'',
                cantidadProductos:'',
                fechaActualizacion:new Date(),
                respuesta01:'',
                mostrartodo:[],
                eqLis01:2,
                errorEncon:'',
                buscar05:'',
                dismissSecs: 10,
                dismissCountDown: 0,
                showDismissibleAlert: false,
                buscarSap02:'',
                ControlSer:true,
                messageUser: '',
                contaTotal:0,
                contaTotalPr:0,
                numcota:1,
                contaTotalPrMate:0,
                incrementar:'',
                listarEquipo:[],
                materialName:'Nombre de material',
                equipoName:'',
                listarEquipoSerie:[],
                stockMaterial:'',
                cantidad:'',
                suma:[],
                sumaTotal:0,
                usuarioName:'',
                tret:0,
                IndexDevo:1,
                IndexLiq:1,
                url:'http://www.eliniktech.com/',

                // devolucion
                vme:0,
                probarinsert:[],
                probarinsertDevolucion:[],
                lista:[],
                success:false,
                arrayListarSerie:[],
                LiquiChecked:[],
                serieInput:'',
                arraySeriesDevolucion:[],
                arrayListarSerieLimite:[],
                serieInputLiqui:'',
                arrayliquida:[],
                nombreusuario:'',
                campoInputdevolucion:'',
                inputbuscarliqui:''

                
                
                
            }
        },
        filters:{
            changeDateFilter:
            function (value){
                    return moment(value).fromNow();
            }
        },

        components:{
            Datepicker,
            vSelect
        },
        computed:{
            error22(){
                return this.buscarSap02 != '' ? false: true
            },
            ConCanti(){
                return this.cantidadProductos <1 ? true : false
            },
            sotState () {
            return this.codigo_sot.length > 2 ? true : false
            },
            cantiState () {
                for (let index = 0; index < this.arraySAPMate.length; index++) {
                    if (this.arraySAPMate[0].ctproduc_stock>=this.cantidadProductos && this.cantidadProductos >0) {
                        this.stockMaterial=this.arraySAPMate[0].ctproduc_stock - this.cantidadProductos;
                        return
                    }else{
                         this.cantidadProductos ='';
                         this.stockMaterial=this.arraySAPMate[0].ctproduc_stock;
                         
                    }
                    
                }
            
            },
            clearSap:function(){

                if (this.buscarSap02!='') {
                    this.buscarSap02='';
                    this.ControlSer=true;
                    this.arrayEquiEn=[];
                    this.eqLis=2;
                    
                }
                else{
                    this.buscarSap02='';
                }
                
            },
            sortOptions () {
            // Create an options list from our fields
            return this.fields
                .filter(f => f.sortable)
                .map(f => { return { text: f.label, value: f.key } })
            },

             isActived: function(){
                return this.pagination.current_page;
            },
            //calcula los elementos de la paginacion
            
             pagesNumber: function() {
                if(!this.pagination.to) {
                    return [];
                }
                
                var from = this.pagination.current_page - this.offset; 
                if(from < 1) {
                    from = 1;
                }

                var to = from + (this.offset * 2); 
                if(to >= this.pagination.last_page){
                    to = this.pagination.last_page;
                }  

                var pagesArray = [];
                while(from <= to) {
                    pagesArray.push(from);
                    from++;
                }
                return pagesArray;             

            }
        },
        methods:{
            controlT(val,val2){

                let resul = 0
               resul= val *100/val2 ;
               return resul;
            },
                showAlert () {
                this.dismissCountDown = this.dismissSecs
                },
             paginador(val) {
                const indiceInicio = (this.currentPage - 1) * this.perPage;
                const indiceFinal =
                    indiceInicio + this.perPage > val.length
                    ? val.length
                    : indiceInicio  + this.perPage;
                return val.slice(indiceInicio , indiceFinal );
                },

            incrementarGuia(){
                let me=this;
               var url='/incrementar-guia' ;
              axios.get(url).then(function (response) {
                   var respuesta=response.data;
                  me.incrementar=respuesta.numero;
               })
               .catch(function (error) {
                   console.log(error);
               });
            },
           exportPdf(){ 
                
                
                let vm = this;
                
        
                let userTo = vm.usuarioName;
                let contaArr = 1
                let imgData = 'data:image/png;base64,'
                // let nomTec = vm.
                let guia= vm.incrementar;
                let fechaImp = moment(this.fechaRegistro).format('DD-MM-YYYY');
                let doc = new jsPDF('p', 'pt');
                let columns = [
                    // {title: "#", dataKey: "1"},
                    {title: "POSI", dataKey: "conta"},
                    
                    {title: "CÓDIGO", dataKey: "sapPdf"},
                    {title: "DESCRIPCIÓN", dataKey: "cate02"},
                    {title: "SERIE", dataKey: "descriPdf"}, 
                    // {title: "Categoria", dataKey: "cate01"},
                    // {title: "Producto", dataKey: "prodescri"},
                    
                    {title: "CANTIDAD", dataKey: "cantidadProducto"},
                    {title: "UDN", dataKey: "unidadDescripcion"},
                ];
                
                    // Error ========================
                    doc.autoTable(columns,vm.tasks,{
                    theme: 'grid',
                    // styles : { fillColor : [ 1, 108 , 162 ] } ,
                    headerStyles: {fillColor : [ 1, 108 , 162 ]},
                    bodyStyles: {fillColor : [ 255, 255 , 255 ]},
                    columnStyles: {
                        id: {fillColor:0}
                    },
                    margin: {top: 170},
                    
                    addPageContent: function(data) {

                        // rectangulo
                        doc.setDrawColor(0);
                        doc.setFillColor(255, 255, 255);
                        doc.roundedRect(410, 35, 140, 55, 3, 3, 'FD'); 

                      

                        // rectangulo FECHA(0,0,ANCHO, ALTURA)
                        doc.setDrawColor(0);
                        doc.setFillColor(255, 255, 255);
                        doc.roundedRect(370, 140, 184, 21, 3, 3, 'FD'); 

                       
                        // nombre tecnico
                        doc.setFontSize(11)
                        doc.setFont('helvetica')
                        doc.setFontType('bold')
                        doc.text("Usuario técnico : "+userTo +"", 45, 155);

                        // fecha
                        doc.setFontSize(11)
                        doc.setFont('helvetica')
                        doc.setFontType('bold')
                        doc.text("FECHA DE EMISIÓN : "+fechaImp +"", 375, 155);
                        // doc.text(fechaImp, 380, 50);
                       
                        // imagen[ 0,0, ancho, alto]
                        doc.addImage(imgData, 'JPEG', 45, 25, 355,80)

                        // ruc
                        doc.setFontSize(12)
                        doc.setFont('helvetica')
                        doc.setFontType('bold')
                        doc.text("R.U.C. : 20600313844", 420, 50);

                        doc.setFontSize(13)
                        doc.setFont('helvetica')
                        doc.setFontType('bold')
                        doc.text("GUIA DE REMISIÓN", 420, 65);
                        // guia
                        doc.text("0001 - "+guia, 420, 85);
                    }
                }
                
                );

                
                doc.save('ELINIKTECH-LIQUIDACION.pdf');
            },


            exportPdfDevolucion(){ 
                
                
                let vm = this;

                let imgData = 'data:image/png;base64,'
                // let nomTec = vm.
                let fechaImp = moment(this.fechaRegistro).format('DD-MM-YYYY');
                let doc = new jsPDF('p', 'pt');
                let columns = [
                    {title: "#", dataKey: "Indi"},
                    {title: "Serie", dataKey: "Numeroserie"},
                    {title: "SAP", dataKey: "sap"},
                    {title: "USUARIO", dataKey: "UserD"},
                    
                ];
                
                    // Error ========================
                    doc.autoTable(columns,vm.probarinsertDevolucion,{
                    theme: 'grid',
                    // styles : { fillColor : [ 1, 108 , 162 ] } ,
                    headerStyles: {fillColor : [ 1, 108 , 162 ]},
                    bodyStyles: {fillColor : [ 255, 255 , 255 ]},
                    columnStyles: {
                        id: {fillColor:0}
                    },
                    margin: {top: 170},
                    
                    addPageContent: function(data) {

                        // rectangulo
                        doc.setDrawColor(0);
                        doc.setFillColor(255, 255, 255);
                        doc.roundedRect(410, 35, 140, 55, 3, 3, 'FD'); 

                      

                        // rectangulo FECHA(0,0,ANCHO, ALTURA)
                        doc.setDrawColor(0);
                        doc.setFillColor(255, 255, 255);
                        doc.roundedRect(370, 140, 184, 21, 3, 3, 'FD'); 



                        // fecha
                        doc.setFontSize(11)
                        doc.setFont('helvetica')
                        doc.setFontType('bold')
                        doc.text("FECHA DE EMISIÓN : "+fechaImp +"", 375, 155);
                        // doc.text(fechaImp, 380, 50);
                       
                        // imagen[ 0,0, ancho, alto]
                        doc.addImage(imgData, 'JPEG', 45, 25, 355,80)

                        // ruc
                        doc.setFontSize(12)
                        doc.setFont('helvetica')
                        doc.setFontType('bold')
                        doc.text("R.U.C. : 20600313844", 420, 50);

                        doc.setFontSize(13)
                        doc.setFont('helvetica')
                        doc.setFontType('bold')
                        doc.text("DEVOLUCIÓN", 420, 65);

                    }
                }
                
                );

                
                doc.save('ELINIKTECH-SALIDA.pdf');
            },
            exportPdfLiquidacion(){ 
                
                
                let vm = this;
                
        
                let userTo = vm.nombreusuario;
                let imgData = 'data:image/png;base64,'
                // let nomTec = vm.
                let guia= vm.incrementar;
                let fechaImp = moment(this.fechaRegistro).format('DD-MM-YYYY');
                let doc = new jsPDF('p', 'pt');
                let columns = [
                    // {title: "#", dataKey: "1"},
                    {title: "POSI", dataKey: "idLiq"},
                    {title: "SAP", dataKey: "sap"},
                    {title: "SERIE", dataKey: "serie"},
                ];
                
                    // Error ========================
                    doc.autoTable(columns,vm.arrayliquida,{
                    theme: 'grid',
                    // styles : { fillColor : [ 1, 108 , 162 ] } ,
                    headerStyles: {fillColor : [ 1, 108 , 162 ]},
                    bodyStyles: {fillColor : [ 255, 255 , 255 ]},
                    columnStyles: {
                        id: {fillColor:0}
                    },
                    margin: {top: 170},
                    
                    addPageContent: function(data) {

                        // rectangulo
                        doc.setDrawColor(0);
                        doc.setFillColor(255, 255, 255);
                        doc.roundedRect(410, 35, 140, 55, 3, 3, 'FD'); 

                      

                        // rectangulo FECHA(0,0,ANCHO, ALTURA)
                        doc.setDrawColor(0);
                        doc.setFillColor(255, 255, 255);
                        doc.roundedRect(370, 140, 184, 21, 3, 3, 'FD'); 

                       
                        // nombre tecnico
                        doc.setFontSize(11)
                        doc.setFont('helvetica')
                        doc.setFontType('bold')
                        doc.text("Usuario técnico : "+userTo +"", 45, 155);

                        // fecha
                        doc.setFontSize(11)
                        doc.setFont('helvetica')
                        doc.setFontType('bold')
                        doc.text("FECHA DE EMISIÓN : "+fechaImp +"", 375, 155);
                        // doc.text(fechaImp, 380, 50);
                       
                        // imagen[ 0,0, ancho, alto]
                        doc.addImage(imgData, 'JPEG', 45, 25, 355,80)

                        // ruc
                        doc.setFontSize(12)
                        doc.setFont('helvetica')
                        doc.setFontType('bold')
                        doc.text("R.U.C. : 20600313844", 420, 50);

                        doc.setFontSize(13)
                        doc.setFont('helvetica')
                        doc.setFontType('bold')
                        doc.text("LIQUIDACIÓN", 420, 65);
                        // guia
                        doc.text("0001 - "+guia, 420, 85);
                    }
                }
                
                );

                
                doc.save('ELINIKTECH-SALIDA.pdf');
            },

            listarSerie(){
                let me=this;
                var url='/todas-series'
               axios.get(url).then(function (response) {
                    var respuesta1=response.data;
                    me.arrayEquiEn=respuesta1.series;
                    me.listarEquipoSerie=respuesta1.series;
                   
                })
                .catch(function (error) {
                    // handle error 
                    console.log(error);
                });
            },
            info (item, index, button) {
         
                           
                    this.listarSerie();

                   this.respuesta = `Row index: ${index}`
            // this.modalInfo.content = item.age
            // this.$root.$emit('bv::show::modal', 'modalInfo', button)
                this.buscar05=item.Serie;
                // this.eqLis=2;

                      for (let index = 0; index < this.arrayEquiEn.length; index++) {
                    const element = this.arrayEquiEn[index].Serie;
                   const estado =this.arrayEquiEn[index].ctdetpr_esta_code;
                        if (element ==this.buscar05 ) 
                        {   
                          if (estado==1) {
                            if (this.buscarArticulo(this.arrayEquiEn[index].Serie)) 
                            {
                                 document.getElementById('xyz').play(); 
                                    swal({
                                        type:'error',
                                        title:'Error...',
                                        text:'Ya se encuentra agregado',
                                })
                            }
                            else{
                                this.tasks.push({
                                    // sap:this.arrayEquiEn[index],
                                    conta: this.numcota++,
                                    detaTi:this.arrayEquiEn[index].ctdetpr_tipro_code,
                                    tipodescri:this.arrayEquiEn[index].cttipro_desc,
                                    cantidadProducto:1,
                                    unidadDescripcion:'Unidad',
                                    cate01:this.arrayEquiEn[index].ctcatego_desc,
                                    cate02:this.arrayEquiEn[index].ctproduc_nombre,
                                    prodescri:this.arrayEquiEn[index].ctproduc_nombre,
                                    prod01:this.arrayEquiEn[index].Serie,
                                    conte: this.arrayEquiEn[index].sap,
                                    uni:this .arrayEquiEn[index].ctundmd_code,
                                    idProduct:this.arrayEquiEn[index].ctproduc_id,

                                    sapPdf: this.arrayEquiEn[index].sap,
                                    descriPdf:this.arrayEquiEn[index].Serie,
                                    // concatenacion
                                   
                                    
                                    
                                });
                                this.contaTotal= this.tasks.length
                                this.arrayListarSerie=='';
                                this.buscar05='';
                                this.materialName='Nombre de material';
                                this.arrayEquiEn;
                             }
                             }else{
                                  document.getElementById('xyz').play(); 
                                 swal({
                                     
                                        type:'error',
                                        title:'Error...',
                                        text:'Ya se encuentra despachado',
                                })
                                this.buscar05='';
                            }
                          } else{
                            if(this.buscar05!='')
                            {
                                 this.timer = setTimeout(() => {
                                     this.showDismissibleAlert=true;
                                   
                                 }, 800);

                            }     
                        }
                }

                
            },
             info01 (item, index, button) {
            this.respuesta = `Row index: ${index}`
                    
                    this.buscar1=item.SAP;
                    this.cantidadProductos='';
                    this.arraySAPMate.length=0;
                    this.materialName='Nombre de material';
                
            },
            listarMaterialB(buscar){
                let me=this;
                
                var url='/materialIngre?buscar=' + buscar ;
               axios.get(url).then(function (response) {
                   
                    var respuesta=response.data;
                   me.arraySAPMate=respuesta.ingresos;
                   console.log(me.arraySAPMate);
                   if (me.arraySAPMate.length < 1) {
                       me.materialName = 'Material no encontrado';
                   } else {
                       me.materialName=me.arraySAPMate[0].ctproduc_nombre;
                   }
                })
                .catch(function (error) {
                    console.log(error);
                });
 
            },
            resetModal () {
            this.modalInfo.title = ''
            this.modalInfo.content = ''
            },
            onFiltered (filteredItems) {
            this.totalRows = filteredItems.length
            this.currentPage = 1
            },
             next01() {
                this.eqLis01=1
   
            },
            next() {
                this.eqLis=1
            },
            showModal () {
            this.refs.myModalRef.show()
            },
            hideModal () {
            this.refs.myModalRef.hide()
            },
            customFormatter(data) {

                    this.fecha1= moment(data).format('LLL');
                    return this.fecha1;   
            },
            deleteTask:function(indice){
                this.tasks.splice(indice,1);
            },
            restantes(val){

               this.docHo= moment(this.fechaActualizacion).format('L');
               return moment(this.docHo).diff(val, 'days');
           },
           
           
           

             mostrarDetalle(){
                let me=this;
                this.listado=0;
                this.listarSerieEquipo();
                me.nroDocumento='';
                me.codigo_sot='';
                me.serie='';
                me.comprobante_id='';
                me.nombre_producto='';
                me.cantidad=0;
                me.precioUnitario=0;
                me.fechaVencimiento='';
                me.tasks=[];
                me.cantidadProductos=this.cantidadProducto;
            },
            ocultarDetalle(){
                 let me=this;
                 if (me.codigo_sot !==''
                 ||me.serie !== ''||me.cantidad !==0||me.buscar1!==''
                 ||me.tasks.length !==0 
                 
                 ) {
                      Swal({
                        title: '¿Seguro que quieres cerrar esta ventana?',
                        type: 'warning',
                        showCancelButton: true,
                        confirmButtonText: 'Aceptar',
                        cancelButtonText: 'Cancelar'
                        }).then((result) => {
                        if (result.value) {
                          this.listado=1;
                        } else if (result.dismiss === Swal.DismissReason.cancel) {
                       
                        }
                        })
                         
                    }else{
                        this.listado=1;
                    }
            },
            selectUsuario(search,loading){
                let me=this;
                loading(true)

                var url= '/user/selectUsuario?filtro='+search;
                axios.get(url).then(function (response) {
                    let respuesta = response.data;
                    q: search
                    me.arrayUsuario=respuesta.usuarios;
                    loading(false)
                })
                .catch(function (error) {
                    console.log(error);
                });
            },
            getDatosProducto(val1){
                let me=this;
                me.loading=true;
                me.usuario_id=val1.ctusuar_code
                me.usuarioName=val1.ctusuar_usuario
            },
             listarProducto(){
                let me=this;
                var url='/producto' ;
               axios.get(url).then(function (response) {
                    var respuesta=response.data;
                   me.arrayProducto1=respuesta.productos.data;
                })
                .catch(function (error) {
                    console.log(error);
                });
            },
            buscarArticulo(id){
                var sw=0;
                for (var i = 0; i < this.tasks.length; i++) {
                    if (this.tasks[i].prod01==id) {
                        sw=true;
                    }
                    
                }
                return sw;

            },
            tipoDe(val){
                if (val==1) {
                    return 'Equipo';
                }else if(val==2){
                    return 'Material';
                }
                else{
                    return '';
                }  
            },
            listarMaterialSap(){
                let me=this;
                var url='/sap/material';
               axios.get(url).then(function (response) {
                    var respuesta=response.data;
                   me.arrayMaterialSap=respuesta.ingresos;
                })
                .catch(function (error) {
                    console.log(error);
                });
               
            },
             
             buscarInfo(){
                //  console.log(this.arrayEquiEn);
                for (let index = 0; index < this.arrayEquiEn.length; index++) {
                    const element = this.arrayEquiEn[index].Serie;
                    const estado =this.arrayEquiEn[index].ctdetpr_esta_code;
                    
                  
                        if (this.buscar05 == element) 
                            {   
                                // console.log(element);
                                
                                if (estado==1) {
                                    if (this.buscarArticulo(this.arrayEquiEn[index].Serie)) 
                                {
                                     document.getElementById('xyz').play(); 
                                        swal({
                                            type:'error',
                                            title:'Error...',
                                            text:'Ya se encuentra agregado',
                                    })
                                    this.buscar05='';
                                }
                                else{
                                    this.tasks.push({
                                        conta: this.numcota++,
                                        detaTi:this.arrayEquiEn[index].ctdetpr_tipro_code,
                                        tipodescri:this.arrayEquiEn[index].cttipro_desc,
                                        cantidadProducto:1,
                                        unidadDescripcion:'Unidad',
                                        cate01:this.arrayEquiEn[index].ctcatego_desc,
                                        cate02:this.arrayEquiEn[index].ctproduc_nombre,
                                        prodescri:this.arrayEquiEn[index].ctproduc_nombre,
                                        prod01:this.arrayEquiEn[index].Serie,
                                        conte: this.arrayEquiEn[index].sap,
                                        uni:this .arrayEquiEn[index].ctundmd_code,
                                        idProduct:this.arrayEquiEn[index].ctproduc_id,
                                        // concatenacion
                                    
                                        sapPdf: this.arrayEquiEn[index].sap,
                                        descriPdf:this.arrayEquiEn[index].Serie,
                                    });
                                    // contador
                                    this.contaTotal= this.tasks.length

                                    this.arrayListarSerie=='';
                                    this.buscar05='';
                                
                                }
                                }else{
                                    document.getElementById('xyz').play(); 
                                    swal({
                                            type:'error',
                                            title:'Error...',
                                            text:'Ya se encuentra despachado',
                                    })
                                    this.buscar05='';
                                }
                                
                            }
                        
                        
                        else{
                            if(this.buscar05!='')
                            {
                                //  this.timer = setTimeout(() => {
                                //      this.showDismissibleAlert=true;
                                   
                                //  }, 800);

                            }
                            this.timer = setTimeout(() => {
                                if(this.buscar05 != element && this.buscar05!='' ){
                                    this.buscar05='';
                                    document.getElementById('xyz').play(); 
                                   swal({
                                        type:'error',
                                        title:'Error...',
                                        text:'Producto no encontrado',
                                })
                                    
                                }
                            }, 800)     
                        }
                        

                   } 
                   
                 

                   
                     
                                   
                   

                 

                

            },
            buscarSap(id){
                var sw=0;
                for (var i = 0; i < this.tasks1.length; i++) {
                    if (this.tasks1[i].prod01==id) {
                        sw=true;
                    }
                    
                }
                return sw;

            },
            listarProductoEncontrar(){
                let me=this;
                var url='/serie/Equi';
               axios.get(url).then(function (response) {
                   
                    var respuesta1=response.data;
                   me.arrayEquiEn=respuesta1.series;
                //    console.log(me.arrayEquiEn);
                //    if (me.arrayEquiEn.length<1) {
                //        me.equipoName='';
                //    } else {
                //        me.equipoName=me.arrayEquiEn[0].ctproduc_nombre;
                //    }
                    
                 me.ControlSer=false;
                //  me.buscarInfo();
                 me.buscar05=''; 
                 me.materialName='Nombre de material'; 
                })
                .catch(function (error) {
                    console.log(error);
                 
                });
            },

             listarSAPMate(buscar1,criterio2){
                let me=this;
                var url='/salida/despacharMateriales?buscar1=' + buscar1 + '&criterio2=' + criterio2;
               axios.get(url).then(function (response) {
                    var respuesta1=response.data;
                   me.arraySAPMate=respuesta1.series.data;
                })
                .catch(function (error) {
                    // handle error 
                    console.log(error);
                });
            },
            pushMaterial(){
                for (let index = 0; index < this.arraySAPMate.length; index++) {
                       const element = this.arraySAPMate[index];


                       if (this.cantidadProductos<=this.arraySAPMate[index].ctproduc_stock){

                           this.tasks.push({
                               cate01:this.arraySAPMate[index].ctcatego_desc,
                               cate02:this.arraySAPMate[index].ctproduc_nombre,
                               prodescri:this.arraySAPMate[index].ctproduc_nombre,
                               conta: this.numcota++,
                               // // CATEGORIA ID Y NOMBRE
                               // categoriaDescripcion:this.arraySAPMate[0].ctcatego_desc,
                               // categoriaID:this.arraySAPMate[0].ctcatego_id,
                               // // PRODUCTO ID Y NOMBRE
                               productoDescripcion:this.arraySAPMate[index].ctproduc_nombre,
                               productoID:this.arraySAPMate[index].ctproduc_id,
                               // // UNIDAD DE MEDIDA ID Y NOMBRE
                               unidadDescripcion:this.arraySAPMate[index].ctundmd_desc,
                               UnidadID:this.arraySAPMate[index].ctproduc_undMed,
                               // concatenacion
                               concaCaDes:(this.arraySAPMate[index].ctcatego_desc)+" | "+(this.arraySAPMate[index].ctproduc_nombre),
                               // // SAP DEL PRODUCTO
                                descriPdf:"ninguno",
                                sapPdf:this.arraySAPMate[index].sap,
                               prod01:this.arraySAPMate[index].sap,
                               sapProducto:this.arraySAPMate[index].sap,
                               // // STOCK DEL PRODUCTO
                               // stockProducto:this.arraySAPMate[0].ctproduc_stock,
                               // //TIPO DE PRODUCTO
                               detaTi:this.arraySAPMate[index].ctproduc_tipro_code,
                               tipodescri:this.arraySAPMate[index].cttipro_desc,

                               // // CANTIDAD DE PRODUCTO QUE SALDRA DE ALMACEN
                               cantidadProducto:this.cantidadProductos,
                               // restar02:(this.arraySAPMate[0].ctproduc_stock)-(this.cantidadProductos)
                                
                           });
                           this.buscar1='';
                           this.cantidadProductos='';
                           this.arraySAPMate='';
                       }
                       else{
                           document.getElementById('xyz').play(); 
                           swal({
                           type:'error',
                           title:'Error...',
                           text:'Cantidad de productos no suficientes',
                           })
                           return this.cantidadProductos='';
                       }
                   }
            },

            agregra(){
                     if (this.stockMate==''||this.stockMate <=0) {
                    
                }
                if (this.tasks.length<1) {
                    var stockMaterial=parseInt(this.arraySAPMate[0].ctproduc_stock) ;
                    var stock=parseInt(this.stockMate)+stockMaterial;
                
                    if (stock>stockMaterial) {
                        document.getElementById('xyz').play(); 
                        swal(`Intenta sobrepasar el limite de stock ${stockMaterial} por ${stock}`)
                        
                 }else{
                     this.pushMaterial();
                 }
                  
                }else{
                    for (let i = 0; i < this.tasks.length; i++) {
                        
                        if (this.tasks[i].prod01==this.buscar1) {
                             this.cantidad=parseInt(this.tasks[i].cantidadProducto)
                             this.sumaTotal+=this.cantidad
                             
                        }
                    }
                  var stockMaterial=parseInt(this.arraySAPMate[0].ctproduc_stock) ;
                  
                  var stock=parseInt(this.cantidadProductos)+parseInt(this.sumaTotal) ;
                 
                   this.sumaTotal=0 
                    if (stock>stockMaterial) {
                        document.getElementById('xyz').play(); 
                        swal(`Intenta sobrepasar el limite de stock ${stockMaterial} por ${stock}`)
                        // console.log(parseInt(this.cantidad),this.sumaTotal);
                 }else{
                        this.pushMaterial();
                        this.sumaTotal=0
                    }
                 }   
                /////////////////////////////////////////////////////////////////////////////////
                   
           },
             deleteTask1:function(indice){
                this.tasks1.splice(indice,1);
            },
             selectCategoria(){
                 let me=this;
                 var url='/categoria/selectCategoria';
               axios.get(url).then(function (response) {
                   var respuesta=response.data;
                   me.arrayCategoria=respuesta.categorias;
                })
                .catch(function (error) {
                    // handle error
                    console.log(error);
                });
            },
            listarSalida(buscar,criterio){
                let me=this;
                var url='/salida/equipo?buscar='+ buscar + '&criterio=' + criterio;
               axios.get(url).then(function (response) {
                    // handle success;
                    var respuesta=response.data;
                   me.arraySalida=respuesta.salidas;
                   me.contaTotalPr= me.arraySalida.length
                })
                .catch(function (error) {
                    // handle error 
                    console.log(error);
                });
            },
           
           
         
            listarSalidaMaterial(buscarSW,criterioSW){
                let me=this;
                var url='/salida/material?buscar='+ buscarSW + '&criterio=' + criterioSW;
               axios.get(url).then(function (response) {
                    // handle success;
                    
                    var respuesta=response.data;
                   me.arraySalidaMaterial=respuesta.salidas;
                   me.contaTotalPrMate= me.arraySalidaMaterial.length
                  
                })
                .catch(function (error) {
                    // handle error 
                    console.log(error);
                });
            },
          
            contarProducto(i) {
                 for (let i = 0; i <= this.tasks.length; i++) {
                        let element = this.tasks[i].length;
                        // console.log(element);
                    }
            },
            metodoRegistrarSalida() {
                 let me=this;
                 axios.post('/salida/registrar',{
                    // 'sot':this.codigo_sot,
                    'fechaLlegada':moment(this.fechaLlegada).format('YYYY-MM-DD hh:mm:ss'),
                    'usuario_id':this.usuario_id,
                    'data':this.tasks,
                    'incrementar':this.incrementar,
                    // 'prueba':this.cantidadProductos
                }).then(function (response) {
                   me.listarSalida('','ctdetsa_serie');
                   me.listarSalidaMaterial('','ctdetsa_serie');
                //    me.tasks=[];
                   me.codigo_sot='';
                   me.label=null;
                //    me.messageUser=me.arrayUsuario[0].ctusuar_usuario;
                   let cantidad =me.tasks.length;
                     toast({
                        type: 'success',
                        title: `Añadido ${cantidad} productos en almacén de ${me.usuarioName}`
                    })
                    me.contaTotal=0;
                    me.tasks=[];
                    me.incrementarGuia();
                    me.listarSerie();
                    me.listarSerieEquipo(me.campoInputdevolucion);
                    me.seriesDevolucion();
                })
                .catch(function (error) {
                    console.log(error);
                });
            },
            registrarSalida(){
                let me=this;
                swal({
                    title: '¿Desea guardar este registro?',
                    
                    type: 'warning',
                    showCancelButton: true,
                    confirmButtonColor: '#3085d6',
                    cancelButtonColor: '#d33',
                    confirmButtonText: 'Aceptar',
                    cancelButtonText: 'Cancelar',
                    }).then((result) => {
                    if (result.value==true) {
                        swal({
                            title: '<strong>¿Desea imprimir el registro?</strong>',
                            type: 'info',
                           
                            showCloseButton: true,
                            showCancelButton: true,
                            focusConfirm: false,
                            confirmButtonText:
                                'Imprimir/Guardar',
                            confirmButtonAriaLabel: 'Thumbs up, great!',
                            cancelButtonText:
                                'Guardar',
                            cancelButtonAriaLabel: 'Thumbs down',
                            }).then((result) => {
                            if (result.value==true) {
                                //  console.log(result.value);
                                me.exportPdf()
                                me.numcota=1;
                                me.metodoRegistrarSalida();
                            } else if(result.dismiss=='cancel') {
                                me.metodoRegistrarSalida();
                                // console.log('cancelado');
                            } else {
                                console.log(result);
                            }
                            })
                           
                          
                    } else {
                        console.log(result);
                    }
                     
                          
                    })
               
                

            },
           
           
            cerrarModal(){
                this.modal=0;
                this.tituloModal='';
                this.idcategoria=0;
                this.descripcion='';
                this.codigo=0;
                this.caracteristicas='';
                this.stock=0;
                this.errorProducto=0;

            },
              cerrarModal1(){
                this.modal=0;
                this.tituloModal='';
                // this.tasks=[];
                // this.newTasks=[];
                this.errorProducto=0;
                

            },
          // Exportar En archivo excel metodo
            pdfExportEquipo(){
                 Swal({
                        title: '¿Desea imprimir?',
                        type: 'success',
                        showCancelButton: true,
                        confirmButtonText: 'Aceptar',
                        cancelButtonText: 'Cancelar',
                        text:'Esto puede tardar'
                        }).then((result) => {
                        if (result.value) {
                        window.open(url+'pdf-salidas');
                        }
                    });
               
              
            },
            // Exportar En archivo excel metodo
            excelExportEquipo(){
                 Swal({
                        title: '¿Desea imprimir?',
                        type: 'success',
                        showCancelButton: true,
                        confirmButtonText: 'Aceptar',
                        cancelButtonText: 'Cancelar',
                        text:'Esto puede tardar'
                        }).then((result) => {
                        if (result.value) {
                        window.open(url+'excel-salidas');
                        }
                    });
               
              
            },
            // Exportar En archivo excel metodo
            pdfExportMaterial(){
                 Swal({
                        title: '¿Desea imprimir?',
                        type: 'success',
                        showCancelButton: true,
                        confirmButtonText: 'Aceptar',
                        cancelButtonText: 'Cancelar',
                        text:'Esto puede tardar'
                        }).then((result) => {
                        if (result.value) {
                        window.open(url+'pdf-salidasM');
                        }
                    });
               
              
            },
            // Exportar En archivo excel metodo
            excelExportMaterial(){
                 Swal({
                        title: '¿Desea imprimir?',
                        type: 'success',
                        showCancelButton: true,
                        confirmButtonText: 'Aceptar',
                        cancelButtonText: 'Cancelar',
                        text:'Esto puede tardar'
                        }).then((result) => {
                        if (result.value) {
                        window.open(url+'excel-salidasM');
                        }
                    });
               
              
            },

        // devolucion

        eliminarIndex: function (index) {

               this.probarinsert.splice(index, 1);
                
            },
            eliminarIndexDevolucion: function (index) {

               this.probarinsertDevolucion.splice(index, 1);
                
            },
            capturaSerie: function (id) {
                if (this.probarinsert < 1) {
                    this.probarinsert.push(id);
                } else {
                    if (this.probarinsert.indexOf(id) === -1) {
                            this.probarinsert.push(id);
                        } else if (this.probarinsert.indexOf(id) > -1) {
                            toast({
                                type: 'warning',
                                title: `la serie ya existe en la colección.`
                            })
                        }
                } 
            },
            capturaSerieDevolucion: function (id) {
                if (this.probarinsertDevolucion < 1) {
                    this.probarinsertDevolucion.push(id);
                } else {
                    if (this.probarinsertDevolucion.indexOf(id) === -1) {
                            this.probarinsertDevolucion.push(id);
                        } else if (this.probarinsertDevolucion.indexOf(id) > -1) {
                            toast({
                                type: 'warning',
                                title: `la serie ya existe en la colección.`
                            })
                        }
                } 
            },
             buscarArticuloDevo(idserie){
                var sw=0;
                for (var i = 0; i < this.probarinsertDevolucion.length; i++) {
                    
                    if (this.probarinsertDevolucion[i].Numeroserie==idserie) {
                        sw=true;
                    }
                    
                }
                return sw;

            },
            seriesDevolucion(){
                 let me=this;
                var url='/series-devolucion' ;
               axios.get(url).then(function (response) {
                //   console.log(response);
                   var respuesta=response.data;
                  me.arraySeriesDevolucion=respuesta.data;
                //   console.log( me.arraySeriesDevolucion);
                 
                })
                .catch(function (error) {
                    // handle error 
                    console.log(error);
                });
            },
            agregarSerie(serie){
                for (let i = 0; i < this.arraySeriesDevolucion.length; i++) {
                    const element =  this.arraySeriesDevolucion[i].ctdetsa_serie;
                    const estado =  this.arraySeriesDevolucion[i].ctdetpr_esta_code;
                    const devolucionUser =  this.arraySeriesDevolucion[i].ctdetsa_traspaso;
                    const sap =  this.arraySeriesDevolucion[i].ctdetsa_sap;
                    
                    if (this.serieInput != null) {
                        if (this.probarinsertDevolucion.length<1) {
                        if (this.serieInput==element) {
                                this.probarinsertDevolucion.push({Numeroserie:this.serieInput,
                                                                    UserD:devolucionUser,
                                                                    sap:sap,
                                                                    Indi:this.IndexDevo++});
                                this.serieInput='';
                                
                           console.log(this.probarinsertDevolucion);
                        }
                    } else {
                        if (this.serieInput==element) {
                            document.getElementById('xyz').play();
                                if (this.buscarArticuloDevo(this.serieInput)) {
                                    swal({
                                        type:'error',
                                        title:'Error...',
                                        text:'Ya se encuentra agregado a la lista',
                                    });
                                    this.serieInput=''
                                    return
                                }else {
                                    this.probarinsertDevolucion.push({Numeroserie:this.serieInput,UserD:devolucionUser,sap:sap,Indi:this.IndexDevo++});
                                    this.serieInput='';
                                    console.log(this.probarinsertDevolucion);
                                    return
                                }
                        }
                         
                    } 
                   
                    }
                    
                       
                }
                
            },
            
            mostrarLista(){
                this.lista = this.arrayEntrada;
            },
            actualizarDevolver(serie){
                let me= this;
                  axios.patch('/devolver/productoArray',{
                                
                        'serie':serie,
                        
                    }).then(function (response) {
                    me.listarSerieEquipo();
                            swal(
                            'Devuelto!',
                            'Registro devuelto',
                            'success'
                            )
                            
                            me.LiquiChecked.length=0 
                            me.probarinsertDevolucion.length=0 
                            me.getDiasdatos(me.inputbuscarliqui);    
                                
                    })
                    
                    .catch(function (error) {
                        console.log(error);
                    });
            },

            devolucionProductos(serie){
                let me=this;
                 if (me.probarinsertDevolucion.length ==0) {
                      toast({
                        type: 'warning',
                        title: `no existe ninguna colección a retornar.`
                    })
                 } else {
                 swal({
                    title: '¿Desea retornar estos registros?',
                    
                    type: 'warning',
                    showCancelButton: true,
                    confirmButtonColor: '#3085d6',
                    cancelButtonColor: '#d33',
                    confirmButtonText: 'Retornar',
                    cancelButtonText: 'Cancelar'
                    }).then((result) => {
                    if (result.value) {
                          if (result.value==true) {
                        swal({
                            title: '<strong>¿Desea imprimir el registro?</strong>',
                            type: 'info',
                           
                            showCloseButton: true,
                            showCancelButton: true,
                            focusConfirm: false,
                            confirmButtonText:
                                'Imprimir/Devolver',
                            confirmButtonAriaLabel: 'Thumbs up, great!',
                            cancelButtonText:
                                'Guardar',
                            cancelButtonAriaLabel: 'Thumbs down',
                            }).then((result) => {
                            if (result.value==true) {
                                //  console.log(result.value);
                                me.exportPdfDevolucion()
                                me.actualizarDevolver(serie);
                            } else if(result.dismiss=='cancel') {
                                me.actualizarDevolver(serie);
                                 console.log('cancelado');
                            } else {
                                console.log(result);
                            }
                            })
                           
                          
                    } else {
                        console.log(result);
                    }
                  
                    }
                    })

                 }
            },

            listarSerieEquipo(campo){
                let me=this;
                var url='/series-salida?data='+campo ;
               axios.get(url).then(function (response) {
                    var respuesta=response.data;
                   me.arrayListarSerie=respuesta.series;
                
                 
                })
                .catch(function (error) {
                    // handle error 
                    console.log(error);
                });
            },
            buscarArticuloBD(id1){
                var sw=0;
                for (var i = 0; i < this.arrayListarSerie.length; i++) {
                    if (this.arrayListarSerie[i].Serie==id1) {
                        sw=true;
                    }
                     
                }
               
                return sw;

            },
            usuariometodo(){
            let me=this;
            var url='/usuario' ;
               axios.get(url).then(function (response) {
                   me.nombreusuario=response.data.user
                })
                .catch(function (error) {
                    // handle error 
                    console.log(error);
                });
        },
        getDiasdatos(input){
            let me=this;
            var url='/fecha-series?data='+input ;
               axios.get(url).then(function (response) {
                   console.log(response);
                   me.arrayListarSerieLimite=response.data.seriesLimite
                   me.usuariometodo();
                })
                .catch(function (error) {
                    // handle error 
                    console.log(error);
                });
        },
        verificarexistliquida(serie){
            let me =this;
            var sw=0;
            for (let i = 0; i < me.arrayliquida.length; i++) {
                if (me.arrayliquida[i].serie==serie) {
                     sw=true;
                }
                
            }
            return sw;
        },
        liquidacionmetodo(serie){
            let me = this;
            for (let i = 0; i <  me.arrayListarSerieLimite.length; i++) {
                const series =  me.arrayListarSerieLimite[i].series;
                const sap =  me.arrayListarSerieLimite[i].sap;
                if (series==serie) {
                    if (me.arrayliquida.length<1) {
                      me.arrayliquida.push({serie:serie,sap:sap,idLiq:this.IndexLiq++});
                      me.serieInputLiqui='';
                    return
                }else{
                    if (me.verificarexistliquida(serie)) {
                        document.getElementById('xyz').play();
                         swal({
                            type:'error',
                            title:'Error...',
                            text:'Ya se encuentra agregado a la lista',
                        });
                        me.serieInputLiqui=''
                        return
                    }else{
                         me.arrayliquida.push({serie:serie,sap:sap,idLiq:this.IndexLiq++});
                        me.serieInputLiqui='';
                        return
                    }
                }
                }
                
                
            }
        },
        eliminarIndexliquida: function (index) {

               this.arrayliquida.splice(index, 1);
                
        },
        liquidarProductoMetodo(series){
            let me=this;
            axios.patch('/liquidar-productoArray',{
                                
                        'series':series,
                        
                    }).then(function (response) {
                            swal(
                            'Liquidado!',
                            'Registro Liquidado',
                            'success'
                            )
                            me.arrayListarSerieLimite.length=0
                            me.arrayliquida.length=0
                            me.arrayListarSerie.length=0;    
                            me.getDiasdatos(me.inputbuscarliqui);
                            me.listarSerieEquipo(me.campoInputdevolucion);
                    })
                    
                    .catch(function (error) {
                        console.log(error);
                    });
        },
        liquidacionProductos(series){
           let me=this;
                 if (me.arrayliquida.length ==0) {
                      toast({
                        type: 'warning',
                        title: `no existe ninguna colección a retornar.`
                    })
                 } else {
                 swal({
                    title: '¿Desea liquidar estos registros?',
                    
                    type: 'warning',
                    showCancelButton: true,
                    confirmButtonColor: '#3085d6',
                    cancelButtonColor: '#d33',
                    confirmButtonText: 'liquidar',
                    cancelButtonText: 'Cancelar'
                    }).then((result) => {
                    if (result.value) {
                          if (result.value==true) {
                        swal({
                            title: '<strong>¿Desea imprimir el registro?</strong>',
                            type: 'info',
                           
                            showCloseButton: true,
                            showCancelButton: true,
                            focusConfirm: false,
                            confirmButtonText:
                                'Imprimir/Guardar',
                            confirmButtonAriaLabel: 'Thumbs up, great!',
                            cancelButtonText:
                                'Liquidar',
                            cancelButtonAriaLabel: 'Thumbs down',
                            }).then((result) => {
                            if (result.value==true) {
                                //  console.log(result.value);
                                me.exportPdfLiquidacion()
                                me.liquidarProductoMetodo(series);
                                
                            } else if(result.dismiss=='cancel') {
                                me.liquidarProductoMetodo(series);
                            } 
                            })
                    }
                    }
                    })

                 }
        }
            
        },
        mounted() {
            // this.listarProductoEncontrar(this.buscarSap02);
            this.listarProductoEncontrar();
            this.listarMaterialSap(1,this.buscar,this.criterio);
            this.listarSalida(this.buscar,this.criterio);
            this.listarSalidaMaterial(this.buscarSW,this.criterioSW);
            this.incrementarGuia();
            this.listarSerie();
            this.getDiasdatos(this.inputbuscarliqui);
            this.listarSerieEquipo(this.campoInputdevolucion);
            this.seriesDevolucion();
        },
        
    }
</script>
<style>
    .equiS{
        font-weight:bold;
        color: green;
    }
    .MateS{
        font-weight:bold;
        color: orange;
    }
    .modal-content{
        width: 100% !important ;
        position: absolute !important;
    }
    .mostrar{
        display: list-item !important;
        opacity: 1 !important ;
        position: absolute !important ;
        background-color: #3c29297a !important;
    }
    .div-error{
        display: flex;
        justify-content: center;
    }
    .text-error{
        color: red !important;
        font-weight: bold;
    }
    .retroCanti{
        background: #cecdcc;
        color: black;
    }
     .card-header .nav.nav-tabs .nav-link, .card-header .nav.nav-tabs .navbar .dropdown-toggle {
        color: #f0f3f5;
    }
     .sim02{
        margin: 0;
        padding: 0;
    }
    .dispaly05{
        position: absolute;
        top:1px;
       right: 1px;

    }
   .gres01{
       background: #fbe7ac!important;
   }
   .reds01{
       font-size: 35px!important;
   }
   .sm-0{
      margin: 0!important;
  }
  .sp-0{
      padding: 0!important;
  }
  .b-01{
      font-weight: bold;
  }
 .bgMa{
      /* background: #5e6d79!important;
      height:100%; */
      width: 100%;
        height: 140px;
        background-size: cover!important;
        background-repeat: no-repeat!important;
        background-position: center center!important;
        padding-bottom: 0px;
  }
  
  .pl-d{
      padding-left: 15px;
  }
  .filesx{
  width: 100%;
  margin-top: 0.25rem;
  font-size: 80%;
  color: #dc3545;
}
.brd{
  border: 1px solid #dc3545!important;
}
.werd{
  border: 1px solid #28a745!important;
}

.brd:focus{
  box-shadow: 0 0 0 0.2rem rgba(146, 15, 15, 0.25)!important;
}
.werd:focus{
  box-shadow: 0 0 0 0.2rem rgba(40, 167, 69, 0.25)!important;
}


</style>
