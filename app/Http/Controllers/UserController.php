...
class UserController extends Controller
{
    public function index(Request $request)
    {
      ...
      if (!$request->ajax()) return redirect('/');
        ...
        if ($buscar=='') {
            ...
            if ($criterio=='ctcolab_ap_paterno') {
                $personas=User::join('permiso','permiso.UserID','=','ctusuar.ctusuar_code')
                ->join('ctcolab','ctcolab.ctcolab_id','=','ctusuar.ctusuar_colab_id')
                ->join('ctcargo','ctcargo.ctcargo_code','=','ctusuar.ctusuar_cargo_code')
                ->select('permiso.idPermiso','permiso.categoria','permiso.equipo','permiso.material','permiso.ingreso',
                ...
                ->orwhere('ctcolab.ctcolab_ap_materno','like','%' .$buscar. '%')
                ->orwhere(DB::raw('CONCAT(ctcolab.ctcolab_ap_paterno," ", ctcolab.ctcolab_ap_materno," ",ctcolab.ctcolab_nombres)'), 'LIKE', '%'.$buscar.'%')
                ->orwhere(DB::raw('CONCAT(ctcolab.ctcolab_nombres," ", ctcolab.ctcolab_ap_paterno," ",ctcolab.ctcolab_ap_materno)'), 'LIKE', '%'.$buscar.'%')
                ->orderby('ctcolab.ctcolab_id','desc')->paginate(10);
            }else{
                $personas=User::join('permiso','permiso.UserID','=','ctusuar.ctusuar_code')
                ->join('ctcolab','ctcolab.ctcolab_id','=','ctusuar.ctusuar_colab_id')
    ...
    //    return $request;
        ...
        if ($cargo==1) {
            ...
            $userCargo->liquidacion=$request->liquidacionChecked;
            $userCargo->UserID=$idUser;
            $userCargo->save();
            // return $userCargo;
       }
