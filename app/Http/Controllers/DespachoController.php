...
class DespachoController extends Controller
{
    ...
    public function almacenTotaEquiposTec(Request $request)
    {
        ...
        if (Auth::user()->ctusuar_cargo_code==1) {
            ...
            if ($buscar=='') {
                ...
                ,'ctdetsa.ctdetsa_colab_id','ctdetsa.ctdetsa_detgu_fecha_reg','ctdetsa.ctdetsa_fecha_reg')
                ...
                //     $data=(int)(((strtotime($this->fechanow())-strtotime($dia->ctdetsa_fecha_reg))/86400));
                //     if ($data>0) {
                //         $fecha[]=['dias'=>$data,'series'=>$dia->ctdetsa_serie,'sap'=>$dia->ctdetsa_sap];
                //     }elseif($data==0){
                //         $fecha;
                //     }
    ...
    public function almacenTotaMaterialesTec(Request $request)
    {
        ...
        if (Auth::user()->ctusuar_cargo_code==1) {
            if ($user==0) {
                if ($buscar=='') {
                    $almacen=DetalleSalida::join('ctproduc','ctproduc.ctproduc_id','=','ctdetsa_roduc_id')
                    ->crossJoin('ctsalid')
                    ->join('ctcolab','ctcolab.ctcolab_id','ctsalid.ctsalid_colab_id')
                    ->join('ctusuar','ctusuar.ctusuar_colab_id','ctcolab.ctcolab_id')
                    ...
                    ,'ctdetsa.ctdetsa_descriTipo','ctdetsa.ctdetsa_fecha_reg','ctproduc.sap')
                ...
                if ($buscar=='') {
                    ...
                    ,'ctdetsa.ctdetsa_descriTipo','ctdetsa.ctdetsa_fecha_reg','ctproduc.sap')
                    ->where('ctdetsa.ctdetsa_tipro_code','=',2)
                    ->where('ctdetsa.ctdetsa_colab_id','=',$user)		
                    ->wherenull('ctdetsa_reportar')
                    ->wherenull('ctdetsa_fechaReporte')
                    ->groupBy('ctdetsa.ctdetsa_indice')
                    ->orderby('ctdetsa.ctdetsa_serie','desc')->paginate(10);
            ...
            // DIFERENTE A ADMINISTRADOR Y ALMACENERO
            if ($buscar=='') {
                $almacen=DetalleSalida::join('ctproduc','ctproduc.ctproduc_id','=','ctdetsa_roduc_id')
                ->crossJoin('ctsalid')
                ->join('ctcolab','ctcolab.ctcolab_id','ctsalid.ctsalid_colab_id')
         ...
         foreach ($detail as $ep => $det) {
            ...
            'ctproduc_stock' => $resta]);
                ...
                if ($cantidad>$stockMaterialReplace) {
                    ...
                    //ACTUALIZAR STOCK DE PRODUCTO SEGUN ID USO
                    $resta=$stockMaterialReplace -$cantidad;
                    $stockUsoAlmacen=ProductoMaster::where('ctproduc_id', $idMaterialDetalle)
                    ->pluck('ctproduc_stock_uso');
                    $stockConvertUso=$stockUsoAlmacen->implode($stockUsoAlmacen);
                    $stockUso=$cantidad+$stockConvertUso;
                    $descontarstockMaterial =ProductoMaster::where('ctproduc_id', $idMaterialDetalle)
                     ->update([
                    'ctproduc_stock' => $resta,'ctproduc_stock_uso' => $stockUso,
                    ]);
                    $salida=DetalleSalida::where('ctdetsa_colab_id',$request->usuario_id)
                    ->where('ctdetsa_sap',$det['sapProducto'])->pluck('ctdetsa_cantidad');
                    if (count($salida)==0) {
                        ...
                        $stockAUser=$salida->implode($salida);
                        $sumaStockM=$stockAUser + $det['cantidadProducto'];
                        $descontarstockMaterial=DetalleSalida::where('ctdetsa_colab_id',$request->usuario_id)
                        ->where('ctdetsa_sap',$det['sapProducto'])
                        ->update([
                        'ctdetsa_cantidad' => $sumaStockM,'ctdetsa_fecha_act' => $dates,
                        ]);
                    }
   ...
   //traslados de Equipos entre Tecnicos
    public function update(Request $request)
    {
        ...
        // return $ObservacionFInal;
            ...
            'ctdetsa_observacion' =>'De: '.Aublueth::user()->ctusuar_usuario.' Para: '.$usuariofinaljson1.' '.
            'Detalle: '.$observacion,
   ...
   public function Numero_de_guia_tecnico()
    ...
    public function pdfTotalSalida()
    {
        $produmaster= DetalleSalida::select(
            'ctdetsa.ctdetsa_sap','ctdetsa.ctdetsa_serie'
            ,'ctdetsa.ctdetsa_cantidad','ctdetsa.ctdetsa_undmdDescri',
            'ctdetsa.ctdetsa_fecha_reg','ctdetsa.ctdetsa_usuario','ctdetsa_produDescri'
            ,'ctdetsa.ctdetsa_traspaso')
            ->wherenull('ctdetsa_reportar')
            ->wherenull('ctdetsa_fechaReporte')
            ->where('ctdetsa_tipro_code','=',1)
            ->orderby('ctdetsa.ctdetsa_indice','asc')->get(); 
        $contarEquipos=count($produmaster);
        $pdf=\PDF::loadView('pdf.salidaEquipoTotalpdf',['salidas'=>$produmaster]);
        return $pdf->download('salidas.pdf');
    }
    public function export() 
    {
        $products = DetalleSalida::select(
            'ctdetsa.ctdetsa_sap','ctdetsa.ctdetsa_serie'
            ,'ctdetsa.ctdetsa_cantidad','ctdetsa.ctdetsa_undmdDescri',
            'ctdetsa.ctdetsa_fecha_reg','ctdetsa.ctdetsa_usuario','ctdetsa_produDescri'
            ,'ctdetsa.ctdetsa_traspaso')
        ...
        Excel::create('Reporte' ,function($excel) use($products) {
            $excel->sheet('Excel sheet', function($sheet) use($products) {
                $sheet->loadView('excel.salidaEquipoTotalxls')->with('salidas',$products);
    ...
    public function pdfTotalSalidaMaterial()
    {
        $produmaster=DetalleSalida::select('ctdetsa.ctdetsa_sot','ctdetsa.ctdetsa_salid_nro',
        'ctdetsa.ctdetsa_sap','ctdetsa.ctdetsa_serie','ctdetsa.ctdetsa_cantidad',
        'ctdetsa.ctdetsa_fecha_reg','ctdetsa.ctdetsa_usuario','ctdetsa.ctdetsa_undmdDescri',
        'ctdetsa.ctdetsa_descriTipo','ctdetsa.ctdetsa_produDescri','ctdetsa.ctdetsa_traspaso')
        ->wherenull('ctdetsa_reportar')
        ->wherenull('ctdetsa_fechaReporte')
        ->where('ctdetsa_tipro_code','=',2)
        ->orderby('ctdetsa.ctdetsa_indice','desc')->get(); 
        $contarEquipos=count($produmaster);
        $pdf=\PDF::loadView('pdf.salidaMaterialTotalpdf',['salidas'=>$produmaster]);
        return $pdf->download('salidas.pdf');
    }
    public function exportMaterial() 
    {
        $products =DetalleSalida::select('ctdetsa.ctdetsa_sot','ctdetsa.ctdetsa_salid_nro',
        'ctdetsa.ctdetsa_sap','ctdetsa.ctdetsa_serie','ctdetsa.ctdetsa_cantidad',
        'ctdetsa.ctdetsa_fecha_reg','ctdetsa.ctdetsa_usuario','ctdetsa.ctdetsa_undmdDescri',
        'ctdetsa.ctdetsa_descriTipo','ctdetsa.ctdetsa_produDescri','ctdetsa.ctdetsa_traspaso')
        ->wherenull('ctdetsa_reportar')
        ...
        Excel::create('Reporte' ,function($excel) use($products) {
            $excel->sheet('Excel sheet', function($sheet) use($products) {
                $sheet->loadView('excel.salidaMaterialTotalxls')->with('salidas',$products);
                $sheet->setOrientation('landscape');
    ...
    public function selectUser()
    {
        ...
        ->select('ctusuar_code','ctusuar_usuario')->get();
        return response()->json(['ok'=>true,'usuarios'=>$user]);
    }
    public function serieDespachado(Request $request)
    {
        return $this->series($request);
    }
     function series($request)
    {
         $idUser=DetalleSalida::where('ctdetsa_colab_id',$request->userid)
        ->where('ctdetsa_tipro_code',1)
        ->select('ctdetsa_serie')->get();
        return response()->json(['ok'=>true,'series'=>$idUser]); 
        
    }
    public function sapStock(Request $request)
    {
        return $this->getSapStock($request);
    }
     function getSapStock( $request)
    {
        $stockSap=DetalleSalida::where('ctdetsa_sap',$request->sap)
        ->where('ctdetsa_colab_id',$request->userid)
        ->select('ctdetsa_cantidad','ctdetsa_produDescri')->get();
        if (count($stockSap)==0) {
            return response()->json(['ok'=>true,'stockSap'=>0],200);
        }else{
            return response()->json(['ok'=>true,'stockSap'=>$stockSap],200);
        }
