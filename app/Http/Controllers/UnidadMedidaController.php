...
class UnidadMedidaController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($buscar=='') {
            $medidas=UnidadMedida::where('ctundmd_code','>',1)
            ->orderby('ctundmd_code','desc')->paginate(10);
        }else {
            $medidas=UnidadMedida::where($criterio,'like','%' .$buscar. '%')
            ->where('ctundmd_code','>',1)
            ->orderby('ctundmd_code')->paginate(10);
        }
